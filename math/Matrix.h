...
struct Matrix {
    ...
    //获取行
    Vector<ncols> GetRow(const int idx) const {
    ...
    //设置行
    void SetRow(const int idx, const Vector<nrows> &v) {
        assert(idx >= 0 && idx < nrows);
        for (int i = 0; i < ncols; ++i) {
            cols[i][idx] = v[i];
        }
    ...
    Matrix<nrows - 1, ncols - 1> GetMinor(const int row, const int col) const {
        ...
        for (int i = 0; i < nrows - 1; ++i) {
            for (int j = 0; j < ncols - 1; ++j) {
                result[i][j] = cols[i < row ? i : i + 1][j < col ? j : j + 1];
            }
        }
        return result;
    }
    ...
    Matrix<nrows, ncols> Invert() const {
        ...
        result = result / result[0].Dot(cols[0]);
...
Vector<nrows> operator*(const Vector<ncols> &lhs, const Matrix<nrows, ncols> &rhs) {
    Vector<nrows> result;
    for (int i = 0; i < nrows; ++i) {
        // result[i] = rhs.GetRow(i) * lhs;
        result[i] = lhs.Dot(rhs.GetRow(i));
    }
    return result;
}
//矩阵乘向量,
...
Matrix<r1, c2> operator*(const Matrix<r1, c1> &lhs, const Matrix<c1, c2> &rhs) {
    ...
    for (int i = 0; i < r1; ++i) {
        for (int j = 0; j < c2; ++j) {
            result[i][j] = lhs.GetRow(j).Dot(rhs[i]);
        }
    }
    return result;
}
...
void TopLeftConner3(Matrix4f &lhs, const Matrix3f &rhs);
Matrix3f VectorMulTranspose(const Vector3f& vec);
} // namespace Maths
#endif